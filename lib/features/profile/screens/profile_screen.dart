...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _deletePhoto(UserModel user) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          "Are you sure you want to remove your profile picture? Default avatar will be restored.",
  ...
  Widget _buildProfileAvatarHeaderCard(UserModel user, bool isMobile) {
    ...
    Color roleColor = CRMColors.primary;
    if (roleLower.contains('super')) {
      roleColor = const Color(0xFF8E24AA); // Purple for Super Admin
          ...
          Row(
            children: [
              Icon(
                ...
                color: isSuperAdmin ? const Color(0xFF8E24AA) : CRMColors.info,
                size: 22,
              ),
              const SizedBox(width: 8),
              Expanded(
          ...
          if (isSuperAdmin) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(CRMBorderRadius.m),
                border: Border.all(color: const Color(0xFF8E24AA).withOpacity(0.2)),
              ),
              child: Column(
                children: [
                  const Icon(
            ...
            Container(
              ...
              child: Row(
                children: [
                  CircleAvatar(
                    backgroundColor: (isAdmin ? const Color(0xFF8E24AA) : CRMColors.info).withOpacity(0.12),
                    radius: 22,
                    child: Icon(
                      isAdmin ? Icons.workspace_premium_rounded : Icons.admin_panel_settings_rounded,
                      color: isAdmin ? const Color(0xFF8E24AA) : CRMColors.info,
                      size: 22,
                    ),
                  ),
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: (isAdmin ? const Color(0xFF8E24AA) : CRMColors.info).withOpacity(0.12),
                      borderRadius: BorderRadius.circular(CRMBorderRadius.round),
                    ),
                    child: Text(
